...
public class MySQLManagerV2 {
    ...
    public MySQLManagerV2(JavaPlugin plugin, String name) {
        ...
        //テーブル作成などはここ
        //出品
        execute("CREATE TABLE if not exists `exhibition` ("
                + "`id` INT NULL,"
                + "`player` VARCHAR(20) NULL DEFAULT NULL,"
                + "`uuid` VARCHAR(50) NULL DEFAULT NULL,"
                + "`item_stack` TEXT NULL DEFAULT NULL,"
                + "`reserve_price` DOUBLE NULL DEFAULT NULL,"
                + "`buyout_price` DOUBLE NULL DEFAULT NULL"
                + "`date` TEXT NULL DEFAULT NULL"
                + ");");

        //入札
        execute("CREATE TABLE if not exists `bid` ("
                + "`id` INT NULL,"
                + "`exhibition_player` VARCHAR(20) NULL DEFAULT NULL,"
                + "`bid_player` VARCHAR(20) NULL DEFAULT NULL,"
                + "`bid_amount` DOUBLE NULL DEFAULT NULL,"
                + "`date` TEXT NULL DEFAULT NULL"
                + ");");

        //出品ログ
        execute("CREATE TABLE if not exists `exhibition_log` ("
                + "`exhibition_player` VARCHAR(20) NULL DEFAULT NULL,"
                + "`item_stack` TEXT NULL DEFAULT NULL,"
        ...
        //入札ログ
        execute("CREATE TABLE if not exists `bid_log` ("
                + "`bid_player` VARCHAR(20) NULL DEFAULT NULL,"
                + "`id` INT NULL,"
                + "`exhibition_player` VARCHAR(50) NULL DEFAULT NULL,"
                + "`bid_amount` DOUBLE NULL DEFAULT NULL,"
                + "`date` TEXT NULL DEFAULT NULL"
                + ");");

        //落札ログ
        execute("CREATE TABLE if not exists `successful_log` ("
                + "`bid_player` VARCHAR(20) NULL DEFAULT NULL,"
                + "`exhibition_player` VARCHAR(20) NULL DEFAULT NULL,"
                + "`item_stack` TEXT NULL DEFAULT NULL,"
        ...
        //プレイヤー所持金
        execute("CREATE TABLE `user_money` ("
                + "`player` VARCHAR(20) NULL DEFAULT NULL,"
                + "`uuid` VARCHAR(50) NULL DEFAULT NULL,"
